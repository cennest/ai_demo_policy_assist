...
class PolicyChatApp:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return """You are a medical policy assistant specialized in analyzing healthcare policy documents and medical necessity criteria.

Your responsibilities:
1. Extract relevant information from provided policy URLs
2. Provide evidence-based answers citing specific policy sections
3. Maintain context from conversation history when relevant
4. Be precise about coverage criteria, requirements, and limitations
5. Always indicate when information is not available in the provided sources

Guidelines:
- Use only information supported by the provided context URLs
- Cite specific policy sections when possible
- Be clear about any limitations or exclusions
...
- Maintain professional medical terminology while being accessible"""
    ...
    def _build_context_prompt(self, user_message: str) -> str:
        ...
        if self.config.url_context_tool and self.config.policy_urls:
            prompt_parts.append("=== POLICY CONTEXT URLS ===")
            prompt_parts.append("Analyze the following policy URLs to answer the user's question:")
